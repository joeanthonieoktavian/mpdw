...
```{r}
data_1 <- read_excel("C:/Users/santo/Documents/S1_Semester 5/mpdw/Pertemuan 2/CES9093161101.xlsx", sheet="Monthly")
head(data_1)
```

## Ambil observasi 172-342

```{r}
data_1 <- data_1 %>%
  filter(Period >=172, Period <=342)
```
...
```{r}
ts.plot(data_1.ts, xlab="Time Period ", ylab="Employees",
...
Pembagian data dilakukan dengan 80% data latih dan 20% data uji
...
# bagi data
training_ma <- data_1[1:n_train, ]
testing_ma  <- data_1[(n_train+1):n, ]
...
plot(data_1.ts, col="red",main="Plot semua data")
points(data_1.ts)
...
Selanjutnya dilakukan peramalan sebanyak 35 periode sesuai dengan jumlah data uji. Pada metode SMA, seluruh hasil peramalan untuk 35 periode ke depan akan memiliki nilai yang sama dengan hasil ramalan satu periode ke depan.
...
```{r}
ts.plot(data_1.ts, xlab="Time Period ", ylab="Employees", main= "SMA N=4 Data Employees")
points(data_1.ts)
...
```{r}
# Pemulusan tahap kedua
dma <- SMA(data.sma, n = 4)

# Menghitung komponen level (At) dan tren (Bt)
At <- 2 * data.sma - dma
Bt <- 2 / (4 - 1) * (data.sma - dma)

# Nilai fitted/ramalan satu periode ke depan
data.dma <- At + Bt
data.ramal2 <- c(NA, data.dma)

# Peramalan 35 periode ke depan
t <- 1:35
f <- c()
...
# bagi data
training <- data_1[1:n_train, ]
testing  <- data_1[(n_train+1):n, ]
...
```{r}
plot(data_1.ts, col="black",main="Plot semua data")
points(data_1.ts)
...
resid2<-training$CES9093161101-ramalan2$fitted
head(resid2)
...
# Selisih antara hasil ramalan dan data aktual
...
plot(
  train.ts,
  main = "Winter Aditif α=0.2; β=0.1; γ=0.1",
  type = "l",
  col = "black",
  xlim = c(1, 171)
)
...
lines(xhat1, col = "red")
lines(xhat1.opt, col = "blue")

lines(
  ts(forecast1, start = 137, frequency = 12),
  col = "red"
)

lines(
  ts(forecast1.opt, start = 137, frequency = 12),
  col = "blue"
)

legend(
  "topleft",
  c("Actual Data", "Winter 0.2;0.1;0.1", "Winter Optimum"),
...
Berdasarkan hasil peramalan pada data uji, metode Single Exponential Smoothing (SES) dengan parameter optimum memberikan kinerja terbaik dibandingkan metode lainnya apabila dilihat berdasarkan nilai MSE dan RMSE. Metode SES optimum menghasilkan RMSE sebesar 40,846 dan MSE sekitar 1.668,47, yang merupakan nilai terkecil dibandingkan metode SMA, DMA, DES, maupun Holt-Winters yang digunakan dalam analisis. Meskipun demikian, metode SMA menghasilkan nilai MAPE sedikit lebih kecil, yaitu sebesar 0,696%, dibandingkan SES optimum sebesar 0,721%. Secara keseluruhan, SES optimum dipilih sebagai metode terbaik karena menghasilkan kesalahan kuadrat rata-rata paling rendah pada data uji. Hal ini menunjukkan bahwa SES optimum memberikan hasil peramalan yang paling mendekati data aktual berdasarkan ukuran MSE dan RMSE.